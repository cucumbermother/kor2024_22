...
public class BoardDao {
    ...
    private BoardDao(){
        try {
            ...
            e.getMessage(); System.out.println("[ BoardDB Connection fail0 ]");
        }
    }
    ...
    public ArrayList<BoardDto> boardPrint( ){
        ArrayList<BoardDto> list = new ArrayList<>();  // 조회된 레코들을 객체화 해서 저장 할 리스트 객체 선언
        try {
            // 1. SQL 작성
            String sql = "select * from board";
            // 2. SQL 기재
            PreparedStatement ps = conn.prepareStatement(sql); // 기재할 SQL 준비
            // 3. SQL 조작 , excuteQuery() : sql 실행 결과 조회된 SQL 조작하는 resultSet 객체 반환
            // 4. SQL 실행
            ResultSet rs =  ps.executeQuery();
            // 5. SQL 결과
            while (rs.next()) { // while(조건){} : 반복문 ,rs.next() : 조회결과에서 다음 레코드 이동 함수
                // 만약에 결과 레코드가 3개 존재하면, rs.next() 3번 실행된다.
                // [해석] 조회 결과 마지막 레코드 까지 하나씩 레코드 이동
                // 6. 레코드를 읽어서 각 필드별 데이터 호출 , rs.get("필드명") : 지정한 필드명의 값 타입에 맞게 값 반환
                int num = rs.getInt("num");       // 현재 조회중인 레코드의 게시물번호(num) 값 호출
                String content = rs.getString("content");// 현재 조회중인 레코드의 게시물번호(content) 값 호출
                String writer = rs.getString("writer"); // 현재 조회중인 레코드의 게시물번호(writer) 값 호출
                int pwd = rs.getInt("pwd"); // 현재 조회중인 레코드의 게시물번호(pwd) 값 호출
                // 7. 각 레코드의 호출된 필드값을 객체화 --> DTO 생성
